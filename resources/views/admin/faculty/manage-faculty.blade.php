...
<div class="space-y-6">
    ...
    <!-- Enterprise Faculty Directory Filter Bar -->
    <form method="GET" action="{{ route('admin.faculty.manage') }}" class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-xs" x-data="{ selectedProgram: '{{ request('program') }}', selectedBranch: '{{ request('branch') }}' }">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-3 pb-3 border-b border-slate-100">
            <div class="flex items-center gap-2">
                ...
                <h4 class="text-xs font-bold uppercase tracking-wider text-slate-800">Faculty Directory Filter Bar</h4>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:flex lg:flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-[180px]">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search faculty name, email or specialization..." class="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-700 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition" onkeydown="if(event.key==='Enter'){this.form.submit();}">
                </div>
            </div>
            ...
            <div class="w-full sm:w-auto">
                <a href="{{ route('admin.faculty.manage') }}" class="w-full sm:w-auto px-4 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition border border-slate-200 flex items-center justify-center gap-1.5">
